// http2-server starts an HTTP/2 (h2c) benchmark server.
package main
...
func main() {
	...
	srv := http2.NewHTTP2Server(transport.TransportConfig{ListenAddr: *addr}, logger)
	...
	for {
		...
		case <-ticker.C:
			s := srv.Stats()
			fmt.Printf("http2-server: conns=%d sent=%d\n", s.Connections, s.Sent)
		}
	}
}
