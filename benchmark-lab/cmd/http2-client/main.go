// http2-client connects to an HTTP/2 benchmark server and receives the stream.
package main
...
func main() {
	...
	rec := metrics.NewRecorder(metrics.RecorderConfig{Label: "http2-client", Protocol: "http2"})
	rec.Start()

	ctx, cancel := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
	defer cancel()

	c := http2.NewHTTP2Client(*addr, func(_ transport.ConnID, data []byte, recvAt time.Time) {
	...
	for {
		...
		case <-ctx.Done():
			snap := rec.Snapshot()
			fmt.Printf("http2-client final: recv=%d msg/s=%.0f\n", snap.MsgRecv, snap.MsgPerSec)
			return
		case <-ticker.C:
			snap := rec.Snapshot()
			fmt.Printf("http2-client: recv=%d msg/s=%.0f\n", snap.MsgRecv, snap.MsgPerSec)
		}
	}
}
